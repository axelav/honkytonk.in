...
renderer.blockquote = function renderBlockquote (text) {
  return `<blockquote class="i ma3 ma5-ns measure">${text}</blockquote>`
}
...
renderer.list = function renderList (body, ordered) {
  ...
  return `<${listType} class="mt0 mb3 pl4 pl5-ns">${body}</${listType}>`
}

renderer.paragraph = function renderParagraph (text) {
  return `<p class="mt0 mb3 measure">${text}</p>`
}
...
renderer.link = function renderLink (href, title, text) {
  if (!title) {
    return `<a href="${href}" class="link blue hover-red">${text}</a>`
  }
